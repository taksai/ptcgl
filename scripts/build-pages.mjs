...
for(const dir of ['src','examples','assets'])await mkdir(new URL(dir+'/',out),{recursive:true});
// Explicit public allowlist: never copy user logs, reference card data, or artwork.
for(const file of ['src/app.js','src/app.css','src/board.js','src/board.css','src/parser.js','src/local-assets.js','examples/demo.txt'])await copyFile(new URL(file,root),new URL(file,out));
